...
from .TDL import TemporalMixer
from .CSP import CSP


class Layer(nn.Module):
    def __init__(self, device, num_nodes, gcn_depth, dropout, residual_channels, conv_channels,
        ...
        self.spatial_mixer = nn.ModuleList([
            CSP(conv_channels, residual_channels, gcn_depth, dropout),
            CSP(conv_channels, residual_channels, gcn_depth, dropout)
        ])
...
class SimST(nn.Module):
    def __init__(self, device, num_nodes, gcn_depth, dropout, input_dim, output_dim,
                 window, horizon, layers, residual_channels, conv_channels, skip_channels,
        ...
        for j in range(1, layers + 1):
            ...
            self.mixer_layers.append(Layer(device=device, num_nodes=num_nodes, gcn_depth=gcn_depth,
                                           dropout=dropout, residual_channels=residual_channels,
