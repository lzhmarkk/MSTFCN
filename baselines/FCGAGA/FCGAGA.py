...
class FcBlock(nn.Module):
    ...
    def forward(self, inputs):
        # inputs: (B, N, C + N + N * T)
        h = self.fc_layers[0](inputs)  # (B, N, C)
        for i in range(1, self.block_layers):
            h = self.fc_layers[i](h)  # (B, N, C)
...
class FcGagaLayer(nn.Module):
    ...
    def forward(self, history_in, time_of_day_in):
        ...
        node_embeddings = self.node_id_em.weight  # (N, C)
        node_id = node_embeddings.unsqueeze(0).repeat(bs, 1, 1)  # (B, N, C)
        ...
        node_embeddings_dp = node_embeddings_dp.unsqueeze(0).unsqueeze(-1)  # (1, N, N, 1)

        assert not torch.isnan(node_embeddings_dp).any()
        assert not torch.isinf(node_embeddings_dp).any()

        level = torch.max(history_in, dim=-1, keepdim=True).values  # (B, N, 1)
        history = self.divide_no_nan(history_in, level)  # (B, N, T)

        assert not torch.isnan(history).any()
        assert not torch.isinf(history).any()
        # Add history of all other nodes
        shape = history_in.shape
        all_node_history = torch.tile(history_in.unsqueeze(1), [1, self.num_nodes, 1, 1])  # (B, N, N, T)

        all_node_history = all_node_history * node_embeddings_dp  # (B, N, N, T)
        all_node_history = all_node_history.reshape(-1, self.num_nodes, self.num_nodes * shape[2])  # (B, N, N * T)
        all_node_history = self.divide_no_nan(all_node_history - level, level)  # (B, N, N * T)

        assert not torch.isnan(all_node_history).any()
        assert not torch.isinf(all_node_history).any()

        all_node_history = fn.relu(all_node_history)  # (B, N, N * T)
        history = torch.concat([history, all_node_history], dim=-1)
        # Add node ID
        history = torch.concat([history, node_id], dim=-1)  # (B, N, C + T + N * T)
...
class FCGAGA(nn.Module):
    ...
    def forward(self, input, **kwargs):
        x = input[..., :-2].permute(0, 2, 1, 3)  # (B, N, T, F)
